...
如果希望当前模块被项目复用或者生成静态或动态库，可以将头文件和源文件分包。
...
在一些规范或者其他地方也许会看到禁止使用goto的条令。

但是本文要求的是，可以使用goto，但是在任何用到goto的地方必须加以大量的充分注释！且保证goto不会出现跳过声明等问题！
...
| 格式                     | 举例                                                         |
| ------------------------ | ------------------------------------------------------------ |
| 全大写+下划线+文件名后缀 | WIDGET_SET_DEVICEMESSAGE_H<br />WIDGET_SET_DEVICEMESSAGE_HPP |
...
但是一般来说多继承会出现很多意想不到的问题，因此不推荐多继承，且推荐直接使用public继承。
...
当设计叫复杂结构，或者有内存管理的时候，应该自定义拷贝和移动语义。
...
但是并不是所有情况都应该用智能指针。很多在使用智能指针的场景，可以用对象+引用的方式代替。
...
| ------ | ------------------------- |
| 数值   | 常量                      |
| 字符串 | const string、const char* |
| 函数   | 模板函数，内联函数        |

## 异常

减少使用。debug时允许少量使用，release尽量都处理掉。

**原因**：

- 异常处理影响性能
- 既然知道此处会有异常，就应该立即测试出来并修改
